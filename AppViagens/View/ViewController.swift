...
class ViewController: UIViewController {
    ...
    func irParaDetalhe(_ viagem: Viagem?) {
        if let viagemSelecionada = viagem {
            let detalhesController = DetalheViewController.instanciar(viagemSelecionada)
            navigationController?.pushViewController(detalhesController, animated: true)
        }
    }
}
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .destaques:
            ...
            guard let cellViagem = tableView.dequeueReusableCell(withIdentifier: "ViagemTableViewCell") as? ViagemTableViewCell else{
                fatalError("erro ao criar tableView cell")
            }
...
extension ViewController: UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            let hearderView = Bundle.main.loadNibNamed("HomeTableViewHeader", owner: self, options: nil)?.first as? HomeTableViewHeader
            hearderView?.configuraView()
            return hearderView
        }
        return nil
    }
